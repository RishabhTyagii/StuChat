...
export default function VerifyOTP() {
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/register/resend-otp/", {
        method: "POST",
  ...
  const handleVerify = async (otpCode?: string) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/register/verify-otp/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-black text-white overflow-hidden relative">
        ...
        <div className="w-full max-w-2xl">
              ...
              <div className="flex justify-center gap-3 md:gap-4 mb-8">
                {otp.map((digit, index) => (
                  <input
                    ...
                    onPaste={handlePaste}
                    className="w-16 h-16 md:w-20 md:h-20 text-3xl font-bold text-center bg-gray-800/50 border-2 border-gray-700 rounded-xl focus:outline-none focus:border-blue-500 transition-all duration-300"
                    disabled={loading}
                  />
                ))}
              </div>
          ...
          <div className="mt-8 text-center">
            <details className="inline-block text-left">
              <summary className="text-gray-400 hover:text-gray-300 cursor-pointer list-none">
      ...
      {/* Add CSS for OTP input focus effect */}
      <style jsx>{`
        input:focus {
